...
import kotlinx.android.synthetic.main.fragment_event_filter_item_list.view.*
...
class FilterItemListFragment : Fragment() {
    ...
    override fun onAttach(context: Context?) {
        ...
        viewModel = when(arguments?.getString("TYPE")) {
            FilterMainMenuFragment.ItemType.Category.toString() -> ViewModelProviders.of(this).get(CategoryListViewModel::class.java)
            else                                                -> ViewModelProviders.of(this).get(VenueListViewModel::class.java)
        }
        if(context is ActionListener) {
            listener = context
        }
        else {
            throw ClassCastException("I need a BaseFilterFragmentListener dumb arse.")
        }
    }

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val rootPOV = inflater.inflate(R.layout.fragment_event_filter_item_list, container, false)
    ...
    private fun setupButtons(rootPOV: View) {
        rootPOV.findViewById<ImageButton>(R.id.backBTN).setOnClickListener {
            listener.showEventFilterMainMenu()
        }
        rootPOV.findViewById<Button>(R.id.doneBTN).setOnClickListener {
    ...
    inner class FilterItemsAdapter : RecyclerView.Adapter<FilterItemsAdapter.FilterItemsVHolder>() {
        ...
        var items: List<FilterItemListViewModel.FilterItem> = listOf()
            set(value) {
                field = value
        ...
        override fun onBindViewHolder(holder: FilterItemsVHolder, position: Int) {
            ...
            holder.selectionToggleBTN.setOnClickListener {
                viewModel.toggleItem(items[position].name)
            }
        }
